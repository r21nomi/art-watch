...
public class JumblrClient {
    ...
    public List<PhotoPost> photos(String blogName, Map<String, ?> options) {
        if (options == null) {
            options = Collections.emptyMap();
        }
        Map<String, Object> soptions = JumblrClient.safeOptionMap(options);
        soptions.put("api_key", this.apiKey);
        return requestBuilder.get(JumblrClient.blogPath(blogName, "/posts/photo"), soptions).getPhotoPosts();
    }
